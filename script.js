...
const CONFIG = {
    ...
    MASTHEAD_BASE: "https://raw.githubusercontent.com/DSCO-Support/JournalMastheads/refs/heads/main/mastheads",
...
function getJournalInfo(infoJson, code) {
    return infoJson.journalInfo?.journal?.[code] ?? infoJson.journal?.[code] ?? null;
}

function getJournalMetrics(metricsJson, code) {
    return metricsJson.journal_metrics?.journal?.[code] ?? metricsJson.journal?.[code] ?? null;
}
...
function buildAffiliation(person) {
    const aff = person?.["Affiliation"] || person?.["Institution"] || person?.["Organization"] || "";
...
function setEditorImage(editorInfo, name, config) {
    ...
    imageEl.src = imgUrl ? CONFIG.ACS_BASE_URL + imgUrl : CONFIG.ACS_BASE_URL + CONFIG.DEFAULT_AVATAR;
...
function adjustEditorVisibility(hasEditorInChief, hasDeputyEditor) {
    ...
    toggleVisibility("#sc-editor-foot__editorInChief-container", hasEditorInChief);
...
async function renderJournal(code, indexes) {
    ...
    if (editorInfo) {
        const fallbackEIC = editorInfo?.["Editor-in-Chief"]?.toString?.() ?? 
                                             (typeof editorInfo?.eic === "string" ? editorInfo.eic : "");
        const fallbackDeputy = editorInfo?.["Deputy Editor"]?.toString?.() ?? 
...
async function render(routeName) {
    try {
        ...
        const infoJournal = indexes.infoIndex.journalInfo?.journal ?? indexes.infoIndex.journal ?? {};
        const metricsJournal = indexes.metricsIndex.journal_metrics?.journal ?? indexes.metricsIndex.journal ?? {};
        ...
        document.getElementById("main").innerHTML = "<h1>Error loading journal data</h1>";
    }
}
